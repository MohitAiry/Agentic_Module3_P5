...
def parse_args(args_str: str):
    """
    Safely parse tool arguments from a string (e.g., from '"India GDP"' to the string "India GDP").
    We use ast.literal_eval which is safer than raw eval().
    """
    try:
...
def run_agent(question: str, max_iterations: int = 8):
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # ==========================================
        # We pass the entire 'context' (which includes the prompt and all past history) to the model
        response = call_llm(context).strip()
        
        # Safety catch: sometimes the LLM tries to generate the observation itself. We cut it off.
        if "Observation:" in response:
            response = response.split("Observation:")[0].strip()
            
        # Immediately append the model's response to our context history
        context += response + "\n"
        
        # ==========================================
        # 2. PARSE THOUGHT: Show the student what the AI is thinking
        # ==========================================
        # Regex explanation: Look for "Thought:" and capture everything until it sees "Action:" or "Answer:"
        ...
        # ==========================================
        # 3. PARSE ACTION: Figure out which tool the AI wants to use
        # ==========================================
        # Regex explanation: Look for "Action: function_name(arguments)"
        ...
        if action_match:
            ...
            # ==========================================
            # 4. OBSERVE: Run the tool and feed the result back
            # ==========================================
            ...
            # Append the observation to the context so the LLM sees it on the next loop
    ...
    print("\nFailed to find an answer within the iteration limit.")
